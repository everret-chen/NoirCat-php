...
| `/sessions` | 会话（设备）管理 |
| `/moderation/reports` | 治理台：举报队列，可筛选状态/原因并结案或忽略（需 `report:handle`） |
| `/moderation/trash` | 回收站：被软删除的帖子与评论，可恢复（版主看全部，成员看自己的） |

帖子页里还有几个就地入口：作者可编辑/删除，**任何成员可举报**，版主在展开的"版主工具"里可置顶 / 加精 / 锁定回复 / 移动版块，评论行内可隐藏、取消隐藏、删除与恢复。
举报只记录"谁报告了什么"，处置动作单独执行，两者都写入 `audit_logs`；举报队列的待处理数量显示在导航栏。
